...
from plugins_base.models import Plugin, APP_TYPES
...
class AddUserToPlugin(GenericAPIView):
    ...
    def post(request):
        ...
        if request.data.get('user') is not None and request.data.get('app') is not None:
            ...
            return Response({"message" : "Unable to grant access to user", "status": "failed"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)

        return Response({"message" : "company or plugin type cannot be missing", "status": "failed"}, status=status.HTTP_400_BAD_REQUEST)
